...
export async function createLocalAttendanceApi(options: { dbName?: string; now?: () => number } = {}): Promise<LocalAttendanceController> {
  ...
  const api: PwaAttendanceApi = {
    ...
    employees: {
      ...
      create: (input) => command('employees:create', input, async () => { const name = input.name.trim(); if (!name || name.length > 80) throw new Error('氏名を入力してください'); if (!Number.isInteger(input.hourlyWage) || input.hourlyWage <= 0 || (input.nightHourlyWage !== undefined && (!Number.isInteger(input.nightHourlyWage) || input.nightHourlyWage <= 0))) throw new Error('時給を確認してください'); const all = await getAll<LocalEmployee>(db, 'employees'); const row: LocalEmployee = { id: uuid(), name, hourly_wage: input.hourlyWage, ...(input.nightHourlyWage !== undefined ? { night_hourly_wage: input.nightHourlyWage } : {}), display_order: Math.max(0, ...all.map((x) => x.display_order ?? 0)) + 1, status: 'ACTIVE', version: 1 }; await putOne(db, 'employees', row); await log('EMPLOYEE_CREATE', row.id, input.requestId); return employeeDto(row); }),
      ...
      bulkUpdateWages: (input) => command('employees:bulk-update-wages', input, async () => { if (!Number.isInteger(input.hourlyWage) || input.hourlyWage <= 0 || !Number.isInteger(input.nightHourlyWage) || input.nightHourlyWage <= 0) throw new Error('時給を確認してください'); const tx = db.transaction(['employees', 'logs'], 'readwrite'); try { const employees = await request<LocalEmployee[]>(tx.objectStore('employees').getAll()); const active = employees.filter((employee) => (employee.status ?? 'ACTIVE') === 'ACTIVE'); for (const employee of active) tx.objectStore('employees').put({ ...employee, hourly_wage: input.hourlyWage, night_hourly_wage: input.nightHourlyWage, version: (employee.version ?? 1) + 1 }); tx.objectStore('logs').put({ id: uuid(), created_at: now(), kind: 'EMPLOYEE_BULK_WAGE_UPDATE', target_id: null, request_id: input.requestId, result: 'SUCCESS' } satisfies LocalLog); await transactionDone(tx); return { updatedCount: active.length }; } catch (error) { try { tx.abort(); } catch { /* Already aborted. */ } throw error; } }),
