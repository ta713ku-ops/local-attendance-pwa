...
import type {
  ...
  CorrectionHistoryDto, CorrectionInput, EmployeeDto, EmployeeWageDefaults, MonthlyEmployeeDetailDto, MonthlyEmployeeDto,
...
const DB_VERSION = 1;
const DEFAULT_EMPLOYEE_WAGE_DEFAULTS: EmployeeWageDefaults = { hourlyWage: 1177, nightHourlyWage: 1471 };
...
type EffectiveRow = { shift: LocalShift; employee: LocalEmployee | undefined; correction: LocalCorrection | undefined; pending: LocalCorrection | undefined; clockIn: number; clockOut: number | null; wage: number; nightWage: number | undefined; method: 'HALF_HOUR' | 'ACTUAL_MINUTES'; longConfirmed: boolean; state: 'OPEN' | 'CALCULATED' | 'NEEDS_REVIEW'; calculation: ReturnType<typeof calculateShift> | null; workDate: string; month: string };

function employeeDto(row: LocalEmployee): EmployeeDto { return { id: row.id, name: row.name, hourlyWage: row.hourly_wage, nightHourlyWage: row.night_hourly_wage ?? null, status: row.status ?? 'ACTIVE', archivedAt: row.archived_at ?? null, restoreUntil: row.restore_until ?? null }; }
...
function exactPay(row: EffectiveRow): PayBreakdownDto | null {
  if (row.calculation?.status !== 'CALCULATED') return null;
  return {
    totalMinutes: row.calculation.totalMinutes,
    regularMinutes: row.calculation.regularMinutes,
    nightMinutes: row.calculation.nightMinutes,
    regularYen: row.calculation.regularPay240thYen / 240,
    nightYen: row.calculation.nightPay240thYen / 240,
    totalYen: row.calculation.pay240thYen / 240,
  };
}
function exactPayTotal(rows: readonly EffectiveRow[]): PayBreakdownDto {
  const calculated = rows.map((row) => row.calculation).filter((value): value is NonNullable<EffectiveRow['calculation']> => value?.status === 'CALCULATED');
  return {
    totalMinutes: calculated.reduce((sum, value) => sum + value.totalMinutes, 0),
    regularMinutes: calculated.reduce((sum, value) => sum + value.regularMinutes, 0),
    nightMinutes: calculated.reduce((sum, value) => sum + value.nightMinutes, 0),
    regularYen: calculated.reduce((sum, value) => sum + value.regularPay240thYen, 0) / 240,
    nightYen: calculated.reduce((sum, value) => sum + value.nightPay240thYen, 0) / 240,
    totalYen: calculated.reduce((sum, value) => sum + value.pay240thYen, 0) / 240,
  };
}
...
function allocateMonthlyPay(rows: EffectiveRow[]): Map<string, PayBreakdownDto> {
...
function validateBackup(value: unknown): LocalBackupPayloadV2 {
  ...
  if (employees.some((x) => { const row = x as LocalEmployee; return !x || typeof x !== 'object' || typeof row.id !== 'string' || typeof row.name !== 'string' || typeof row.hourly_wage !== 'number' || (row.night_hourly_wage !== undefined && typeof row.night_hourly_wage !== 'number'); })) throw new Error('従業員データが壊れています');
  if (shifts.some((x) => { const row = x as LocalShift; return !x || typeof x !== 'object' || typeof row.id !== 'string' || typeof row.employee_id !== 'string' || typeof row.clock_in !== 'number' || typeof row.wage_snapshot !== 'number' || (row.night_wage_snapshot !== undefined && typeof row.night_wage_snapshot !== 'number') || (row.voided_at !== undefined && row.voided_at !== null && typeof row.voided_at !== 'number') || (row.void_reason !== undefined && row.void_reason !== null && typeof row.void_reason !== 'string'); })) throw new Error('勤務データが壊れています');
  if (stores.meta.some((x) => !x || typeof x !== 'object' || typeof (x as { key?: unknown }).key !== 'string' || !('value' in x))) throw new Error('管理情報が壊れています');
  if (stores.corrections.some((x) => { const row = x as Partial<LocalCorrection>; return !x || typeof x !== 'object' || typeof row.id !== 'string' || typeof row.shift_id !== 'string' || typeof row.start_at !== 'number' || typeof row.end_at !== 'number' || (row.night_hourly_wage !== undefined && typeof row.night_hourly_wage !== 'number') || !['PENDING', 'APPROVED', 'REJECTED'].includes(row.status ?? '') || typeof row.created_at !== 'number'; })) throw new Error('訂正データが壊れています');
...
export async function createLocalAttendanceApi(options: { dbName?: string; now?: () => number } = {}): Promise<LocalAttendanceController> {
  ...
  const currentEmployeeWageDefaults = async (): Promise<EmployeeWageDefaults> => {
    const stored = await meta<EmployeeWageDefaults>(db, 'employeeWageDefaults');
    return stored && Number.isInteger(stored.hourlyWage) && stored.hourlyWage > 0 && Number.isInteger(stored.nightHourlyWage) && stored.nightHourlyWage > 0
      ? stored
      : DEFAULT_EMPLOYEE_WAGE_DEFAULTS;
  };
  // Authentication remains valid for the lifetime of this controller.  It is
  ...
  const rows = async (): Promise<EffectiveRow[]> => {
    ...
    return shifts.filter((shift) => shift.voided_at == null).map((shift) => {
      ...
      const clockIn = correction?.start_at ?? shift.clock_in; const clockOut = correction?.end_at ?? shift.clock_out ?? null; const wage = correction?.hourly_wage ?? shift.wage_snapshot; const nightWage = correction?.night_hourly_wage ?? shift.night_wage_snapshot; const method = correction?.calculation_method ?? (exceptions.some((x) => x.shift_id === shift.id) ? 'ACTUAL_MINUTES' : 'HALF_HOUR'); const longConfirmed = correction?.long_shift_confirmed ?? exceptions.some((x) => x.shift_id === shift.id);
      const calculation = clockOut == null ? null : calculateShift({ clockIn, clockOut, hourlyWageYen: wage, nightHourlyWageYen: nightWage, approvedActualMinutes: method === 'ACTUAL_MINUTES', approvedLongShiftReview: longConfirmed });
      return { shift, employee: employeeMap.get(shift.employee_id), correction, pending, clockIn, clockOut, wage, nightWage, method, longConfirmed, state: clockOut == null ? 'OPEN' : calculation!.status, calculation, workDate: formatJstDate(clockIn), month: formatJstMonth(clockIn) };
    });
  };
  ...
  const api: PwaAttendanceApi = {
    clock: {
      ...
      clockIn: (input) => command('clock:in', input, async () => { const employee = await getOne<LocalEmployee>(db, 'employees', input.employeeId); if (!employee || employee.status === 'ARCHIVED') throw new Error('従業員が見つかりません'); if (await periodClosed(formatJstMonth(now()))) throw new Error('月締め済みです'); const shifts = (await getAll<LocalShift>(db, 'shifts')).filter((x) => x.voided_at == null); if (shifts.some((x) => x.employee_id === employee.id && x.clock_out == null)) throw new Error('すでに出勤中です'); if (shifts.some((x) => x.employee_id === employee.id && x.clock_out != null && formatJstDate(x.clock_in) === formatJstDate(now())) && !input.reClockAcknowledged) throw new Error('本日は退勤済みです。再出勤を確認してください'); const at = now(); const shift: LocalShift = { id: uuid(), employee_id: employee.id, business_date: formatJstDate(at), clock_in: at, clock_out: null, wage_snapshot: employee.hourly_wage, ...(employee.night_hourly_wage !== undefined ? { night_wage_snapshot: employee.night_hourly_wage } : {}), calc_status: 'OPEN', created_at: at }; await putOne(db, 'shifts', shift); await log('CLOCK_IN', employee.id, input.requestId); return { shiftId: shift.id, employeeId: employee.id, businessDate: formatJstDate(at), clockIn: at, clockOut: null, state: 'OPEN' as const }; }, false),
      clockOut: (input) => command('clock:out', input, async () => { const shift = (await getAll<LocalShift>(db, 'shifts')).find((x) => x.employee_id === input.employeeId && x.clock_out == null && x.voided_at == null); if (!shift) throw new Error('出勤中の勤務がありません'); if (await periodClosed(formatJstMonth(shift.clock_in))) throw new Error('月締め済みです'); if (now() <= shift.clock_in) throw new Error('端末時刻を確認してください'); const clockOut = now(); const calculation = calculateShift({ clockIn: shift.clock_in, clockOut, hourlyWageYen: shift.wage_snapshot, nightHourlyWageYen: shift.night_wage_snapshot }); const updated = { ...shift, clock_out: clockOut, calc_status: calculation.status }; await putOne(db, 'shifts', updated); await log('CLOCK_OUT', input.employeeId, input.requestId); return { shiftId: shift.id, employeeId: shift.employee_id, businessDate: formatJstDate(shift.clock_in), clockIn: shift.clock_in, clockOut, state: calculation.status }; }, false),
    },
    adminAuth: {
      setup: (input) => command('auth:setup', input, async () => { validatePin(input.pin); if (await meta(db, 'administrator')) throw new Error('管理者は設定済みです'); const recoveryCode = randomSecret(); await setMeta(db, 'administrator', { pinHash: await hashSecret(input.pin), recoveryHash: await hashSecret(recoveryCode), displayName: input.displayName?.trim() || '管理者', failedCount: 0, lockedUntil: 0 } satisfies AdminRecord); adminAuthenticated = true; return { recoveryCode }; }, false, false),
      verify: async (input) => { try { await verifyAdminSecret(input.pin); adminAuthenticated = true; return ok({ authenticatedUntil: Number.MAX_SAFE_INTEGER }); } catch (error) { return fail(error); } },
      lock: async () => { adminAuthenticated = false; },
      changePin: (input) => command('auth:change', input, async () => { validatePin(input.newPin); const admin = await meta<AdminRecord>(db, 'administrator'); if (!admin || !(await checkSecret(input.oldPin, admin.pinHash))) throw new Error('現在のPINが正しくありません'); await setMeta(db, 'administrator', { ...admin, pinHash: await hashSecret(input.newPin), failedCount: 0, lockedUntil: 0 }); adminAuthenticated = true; return { changed: true }; }),
      resetWithRecovery: (input) => command('auth:recover', input, async () => { validatePin(input.newPin); const admin = await meta<AdminRecord>(db, 'administrator'); if (!admin || !(await checkSecret(input.recoveryCode, admin.recoveryHash))) throw new Error('回復コードが正しくありません'); const recoveryCode = randomSecret(); await setMeta(db, 'administrator', { ...admin, pinHash: await hashSecret(input.newPin), recoveryHash: await hashSecret(recoveryCode), failedCount: 0, lockedUntil: 0 }); adminAuthenticated = true; return { recoveryCode }; }, false, false),
    },
    employees: {
      ...
      defaults: () => withRead(() => currentEmployeeWageDefaults()),
      create: (input) => command('employees:create', input, async () => { const defaults = await currentEmployeeWageDefaults(); const hourlyWage = input.hourlyWage ?? defaults.hourlyWage; const nightHourlyWage = input.nightHourlyWage ?? defaults.nightHourlyWage; const name = input.name.trim(); if (!name || name.length > 80) throw new Error('氏名を入力してください'); if (!Number.isInteger(hourlyWage) || hourlyWage <= 0 || !Number.isInteger(nightHourlyWage) || nightHourlyWage <= 0) throw new Error('時給を確認してください'); const all = await getAll<LocalEmployee>(db, 'employees'); const row: LocalEmployee = { id: uuid(), name, hourly_wage: hourlyWage, night_hourly_wage: nightHourlyWage, display_order: Math.max(0, ...all.map((x) => x.display_order ?? 0)) + 1, status: 'ACTIVE', version: 1 }; await putOne(db, 'employees', row); await log('EMPLOYEE_CREATE', row.id, input.requestId); return employeeDto(row); }),
      update: (input) => command('employees:update', input, async () => { const row = await getOne<LocalEmployee>(db, 'employees', input.id); if (!row) throw new Error('従業員が見つかりません'); if (input.name !== undefined && !input.name.trim()) throw new Error('氏名を入力してください'); if ((input.hourlyWage !== undefined && (!Number.isInteger(input.hourlyWage) || input.hourlyWage <= 0)) || (input.nightHourlyWage !== undefined && (!Number.isInteger(input.nightHourlyWage) || input.nightHourlyWage <= 0))) throw new Error('時給を確認してください'); await putOne(db, 'employees', { ...row, ...(input.name !== undefined ? { name: input.name.trim() } : {}), ...(input.hourlyWage !== undefined ? { hourly_wage: input.hourlyWage } : {}), ...(input.nightHourlyWage !== undefined ? { night_hourly_wage: input.nightHourlyWage } : {}), version: (row.version ?? 1) + 1 }); await log('EMPLOYEE_UPDATE', input.id, input.requestId); return { id: input.id }; }),
      bulkUpdateWages: (input) => command('employees:bulk-update-wages', input, async () => { if (!Number.isInteger(input.hourlyWage) || input.hourlyWage <= 0 || !Number.isInteger(input.nightHourlyWage) || input.nightHourlyWage <= 0) throw new Error('時給を確認してください'); const tx = db.transaction(['employees', 'logs', 'meta'], 'readwrite'); try { const employees = await request<LocalEmployee[]>(tx.objectStore('employees').getAll()); const active = employees.filter((employee) => (employee.status ?? 'ACTIVE') === 'ACTIVE'); for (const employee of active) tx.objectStore('employees').put({ ...employee, hourly_wage: input.hourlyWage, night_hourly_wage: input.nightHourlyWage, version: (employee.version ?? 1) + 1 }); tx.objectStore('meta').put({ key: 'employeeWageDefaults', value: { hourlyWage: input.hourlyWage, nightHourlyWage: input.nightHourlyWage } satisfies EmployeeWageDefaults }); tx.objectStore('logs').put({ id: uuid(), created_at: now(), kind: 'EMPLOYEE_BULK_WAGE_UPDATE', target_id: null, request_id: input.requestId, result: 'SUCCESS' } satisfies LocalLog); await transactionDone(tx); return { updatedCount: active.length }; } catch (error) { try { tx.abort(); } catch { /* Already aborted. */ } throw error; } }),
    ...
    attendance: {
      ...
      previewCorrection: (input) => withRead(async () => { const before = await correctionDetail(input.shiftId); if (input.endAt <= input.startAt) throw new Error('退勤は出勤より後にしてください'); if (!Number.isInteger(input.hourlyWage) || input.hourlyWage <= 0 || (input.nightHourlyWage !== undefined && (!Number.isInteger(input.nightHourlyWage) || input.nightHourlyWage <= 0))) throw new Error('時給を確認してください'); const current = await rows(); const original = current.find((x) => x.shift.id === input.shiftId); if (!original) throw new Error('勤務が見つかりません'); const nightWage = input.nightHourlyWage ?? original.nightWage; const calculation = calculateShift({ clockIn: input.startAt, clockOut: input.endAt, hourlyWageYen: input.hourlyWage, nightHourlyWageYen: nightWage, approvedActualMinutes: input.calculationMethod === 'ACTUAL_MINUTES', approvedLongShiftReview: input.longShiftConfirmed }); const temp: EffectiveRow = { ...original, clockIn: input.startAt, clockOut: input.endAt, wage: input.hourlyWage, nightWage, method: input.calculationMethod, longConfirmed: input.longShiftConfirmed, state: calculation.status, calculation, workDate: formatJstDate(input.startAt), month: formatJstMonth(input.startAt) }; return { before, after: { workDate: temp.workDate, month: temp.month, pay: exactPay(temp), state: temp.state } }; }),
      applyCorrection: (input) => command('attendance:apply-correction', input, async () => { const shift = await getOne<LocalShift>(db, 'shifts', input.shiftId); if (!shift || shift.voided_at != null) throw new Error('勤務が見つかりません'); if (input.endAt <= input.startAt) throw new Error('退勤は出勤より後にしてください'); if (!Number.isInteger(input.hourlyWage) || input.hourlyWage <= 0 || (input.nightHourlyWage !== undefined && (!Number.isInteger(input.nightHourlyWage) || input.nightHourlyWage <= 0))) throw new Error('時給を確認してください'); const existing = (await getAll<LocalCorrection>(db, 'corrections')).filter((x) => x.shift_id === shift.id); const previous = latestApproved(existing); const effectiveNightWage = input.nightHourlyWage ?? previous?.night_hourly_wage ?? shift.night_wage_snapshot; const oldMonth = formatJstMonth(previous?.start_at ?? shift.clock_in); const newMonth = formatJstMonth(input.startAt); if (await periodClosed(oldMonth) || await periodClosed(newMonth)) throw new Error('移動元または移動先の月が確定済みのため訂正できません'); const row: LocalCorrection = { id: uuid(), shift_id: shift.id, start_at: input.startAt, end_at: input.endAt, hourly_wage: input.hourlyWage, ...(effectiveNightWage !== undefined ? { night_hourly_wage: effectiveNightWage } : {}), reason: input.reason?.trim() ?? '', calculation_method: input.calculationMethod, long_shift_confirmed: input.longShiftConfirmed, status: 'APPROVED', applied_at: now(), created_at: now() }; await putOne(db, 'corrections', row); await log('CORRECTION_APPLY', shift.id, input.requestId); return correctionDetail(shift.id); }),
    ...
    monthly: {
      ...
      employeeDetail: (month, employeeId) => withRead(async (): Promise<MonthlyEmployeeDetailDto> => { const totals = (await summary(month)).employees.find((x) => x.employeeId === employeeId); const employee = await getOne<LocalEmployee>(db, 'employees', employeeId); if (!employee || !totals) throw new Error('従業員の月次データが見つかりません'); const effective = (await rows()).filter((x) => x.shift.employee_id === employeeId && x.month === month); const shifts = (await dtoRows()).filter((x) => x.employeeId === employeeId && x.workDate.startsWith(month)); const days = [...new Set(shifts.map((x) => x.workDate))].sort().map((date) => { const own = shifts.filter((x) => x.workDate === date); return { date, shifts: own, totals: exactPayTotal(effective.filter((x) => x.workDate === date)) }; }); return { month, employee: employeeDto(employee), totals, days }; }),
